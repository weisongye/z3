...
--*/
#include"pp_solver_adapter.h"
#include"solver_exception.h"
#include"tactic.h"
#include"params.h"
#include"scoped_ptr_vector.h"

struct pp_solver_adapter::imp {
    ...
    void commit() {
        ...
        if (!m_stack.inconsistent()) {
            ...
            for (unsigned i = qhead; i < sz; i++) {
                ...
                m_kernel->assert_expr(e, m_stack.pr(i));
            }
        }
    }
...
pp_solver_adapter::pp_solver_adapter(ast_manager & m, params_ref const & p, bool produce_proofs, bool produce_models, bool produce_unsat_cores) {
    m_imp = alloc(imp, m, p, produce_proofs, produce_models, produce_unsat_cores);
}

pp_solver_adapter::~pp_solver_adapter() {
    dealloc(m_imp);
}

void pp_solver_adapter::collect_param_descrs(param_descrs & r) {
    // do nothing
    m_imp->m_kernel->collect_param_descrs(r);
}

void pp_solver_adapter::set_produce_models(bool f) {
    m_imp->m_kernel->set_produce_models(f);
}

void pp_solver_adapter::set_progress_callback(progress_callback * callback) {
    m_imp->m_kernel->set_progress_callback(callback);
}

void pp_solver_adapter::updt_params(params_ref const & p) {
    m_imp->updt_params(p);
}

void pp_solver_adapter::set_cancel(bool f) {
    m_imp->set_cancel(f);
}

unsigned pp_solver_adapter::get_num_assertions() const {
    return m_imp->m_stack.size();
}

expr * pp_solver_adapter::get_assertion(unsigned idx) const {
    return m_imp->m_stack.form(idx);
}

void pp_solver_adapter::display(std::ostream & out) const {
    m_imp->m_stack.display(out);
}

unsigned pp_solver_adapter::get_scope_level() const {
    return m_imp->m_stack.scope_lvl();
}

void pp_solver_adapter::push() {
    m_imp->push();
}

void pp_solver_adapter::pop(unsigned n) {
    m_imp->pop(n);
}

void pp_solver_adapter::assert_expr(expr * t, expr * a) {
    m_imp->assert_expr(t, a);
}

void pp_solver_adapter::assert_expr(expr * t) {
    m_imp->assert_expr(t);
}

lbool pp_solver_adapter::check_sat(unsigned num_assumptions, expr * const * assumptions) {
    return m_imp->check_sat(num_assumptions, assumptions);
}

void pp_solver_adapter::collect_statistics(statistics & st) const {
    m_imp->collect_statistics(st);
}

void pp_solver_adapter::get_unsat_core(ptr_vector<expr> & r) {
    m_imp->get_unsat_core(r);
}

void pp_solver_adapter::get_model(model_ref & md) {
    m_imp->get_model(md);
}

proof * pp_solver_adapter::get_proof() {
    return m_imp->get_proof();
}

std::string pp_solver_adapter::reason_unknown() const {
    return m_imp->reason_unknown();
}

void pp_solver_adapter::get_labels(svector<symbol> & r) {
    throw solver_exception("Solver does not support get_labels");
}

void pp_solver_adapter::freeze(func_decl * f) {
    m_imp->m_stack.freeze(f);
}

struct pp_solver_adapter_factory::imp {
