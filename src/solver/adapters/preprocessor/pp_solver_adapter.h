...
--*/
#ifndef _PP_SOLVER_ADAPTER_H_
#define _PP_SOLVER_ADAPTER_H_

#include"solver.h"
class tactic_factory;


class pp_solver_adapter : public solver {
    struct imp;
    imp *  m_imp;
    friend class solver_factory;
public:
    pp_solver_adapter(ast_manager & m, solver * s, params_ref const & p, bool produce_proofs, bool produce_models, bool produce_unsat_cores);
    virtual ~solver();
