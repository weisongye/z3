...
void mc_context::display(std::ostream & out, term_cond * tc) {
    out << "(";
    for( unsigned i=0; i<tc->get_size(); i++ ){
        if(i>0) out << ", ";
...
bool mc_context::repair_formula(model_constructor * mct, quantifier * q, expr * e, ptr_buffer<val> & vsub, expr_ref_buffer & tsub, bool polarity) {
    ...
    if (is_app(e)) {
        ...
        else if (m_m.is_eq(e) || m_m.is_iff(e)) {
            for (unsigned i=0; i<2; i++) {
                expr * ec = to_app(e)->get_arg(i);
...
bool mc_context::repair_term(model_constructor * mct, quantifier * q, expr * t, ptr_buffer<val> & vsub, expr_ref_buffer & tsub, val * v) {
    ...
    SASSERT(df->is_annotated_simple());
    annotated_simple_def * sdf = to_annotated_simple(df);
    if (!sdf->evaluate(*this, args, true)) {
        ...
        if (vsub.size()==inst.size()) {
            ...
            if (addInstantiation && filterRepair && m_model_repairing) {
                ...
                if (repair_formula(mct, q, q->get_expr(), vsub, inst, true)) {
                    ...
                    //val * v2 = evaluate(mct, q->get_expr(), vsub);
                    //SASSERT(v2);
                    //SASSERT(m_m.is_true(to_expr(v2)->get_value()));
                }
                else {
        ...
        if (is_ground(e)) {
            ...
            if (is_uninterp(f)) {
                ...
                if (!var_to_bind.empty()) {
                    ...
                    SASSERT(df->is_annotated_simple());
                    SASSERT(to_annotated_simple(df)->m_else);
                    ...
                    for (unsigned i=0; i<(df->get_num_entries()-1); i++) {
                        ...
                        if (do_compose(vsub, children, cf, esub, tcf)) {
                            ...
                            if (var_bind_count<q->get_num_decls()) {
                                ...
                                if (add_instantiation(mct, q, esub, vsub, instantiations, repaired, true, true, false)) {
                                    eresult = l_true;
                                }
