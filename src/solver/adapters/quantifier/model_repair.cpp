    ...
    if (is_app(e)) {
        ...
        if ((m_m.is_or(e) && polarity) || (m_m.is_and(e) && !polarity)) {
            ...
            for (unsigned i=0; i<children_undef.size(); i++) {
                if (do_repair_formula(mc, mct, q, to_app(e)->get_arg(children_undef[i]), vsub, tsub, polarity, fail_entry, fail_value, fail_func, inst_reason)) {
                    return true;
                }
            }
            return false;
            ...
            //only chance is to look at partial model (this should not be necessary)
            //std::cout << "Don't know how to repair " << mk_pp(e,m_m) << "\n";
            //std::cout << (ecv==0) << " " << m_m.is_true(ecv) << " " << m_m.is_false(ecv) << " " << polarity << "\n";
